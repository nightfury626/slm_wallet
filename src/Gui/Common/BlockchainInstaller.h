...
private:
  ...
  const QDir m_siliconlumenDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfSiliconLumenBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _siliconlumen) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo siliconlumenBlockchainInfo() const;
