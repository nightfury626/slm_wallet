...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_siliconlumenDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfSiliconLumenBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 siliconlumenHeight;
  if (!checkIfBlockchainOutdated(currentHeight, siliconlumenHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(siliconlumenHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfSiliconLumenBlockchainExists() const {
  return m_siliconlumenDir.exists() && m_siliconlumenDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_siliconlumenDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::siliconlumenBlockchainInfo() const {
  return QFileInfo(m_siliconlumenDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_siliconlumenDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_siliconlumenDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
