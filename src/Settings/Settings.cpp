...
const char DEFAULT_WALLET_FILE_NAME[] = "siliconlumen.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "165.227.237.55:13254";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("siliconlumenwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("siliconlumenwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("siliconlumenwallet.desktop");
  ...
  res = autorunSettings.contains("SiliconLumenWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("SiliconLumenWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.siliconlumen.siliconlumenwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=SiliconLumen Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("SiliconLumenWallet", appPath);
    } else {
      autorunSettings.remove("SiliconLumenWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\siliconlumen", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:siliconlumen");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\siliconlumen\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\siliconlumen\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("siliconlumenwallet.cfg"));
